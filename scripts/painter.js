...
var rootref = firebase.database().ref().child("painter");
...
rootref.on("child_added", snap =>{
    ...
    let usersRef2 = firebase.database().ref('Services/painter');
    ...
    usersRef.orderByValue().on("value", function(snapshot) {
        ...
        // current_id = snapshot.val().painter.id;
        snapshot.forEach(function(data) {
            var data_id = data.val().id;
            // console.log(data.val())
            var all_data = data.val();
            // if(data.val().name !== "painter"){
            // console.log("i ma here")

            // }
            
            if(current_id === data_id && data.val().name !== "painter"){
                let div = document.createElement('div');
...
var formref = firebase.database().ref('painter');
...
const close_rating = (rating_block, code_arrived) =>{
    ...
    var ref = firebase.database().ref('painter');
    console.log(ref);
...
const writeUserData = (userId ,code_send, email_send, message_send, name_send, phone_send, orignal, img_send) => {
    firebase.database().ref(`painter/${userId}`).set({
