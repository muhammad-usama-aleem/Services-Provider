...
var rootref = firebase.database().ref().child("medical laboratory");
...
rootref.on("child_added", snap =>{
    ...
    let usersRef2 = firebase.database().ref('Services/medical laboratory');
    ...
    usersRef.orderByValue().on("value", function(snapshot) {
        ...
        // current_id = snapshot.val().medical laboratory.id;
        snapshot.forEach(function(data) {
            var data_id = data.val().id;
            // console.log(data.val())
            var all_data = data.val();
            // if(data.val().name !== "medical laboratory"){
            // console.log("i ma here")

            // }
            
            if(current_id === data_id && data.val().name !== "medical laboratory"){
                let div = document.createElement('div');
...
var formref = firebase.database().ref('medical laboratory');
...
const close_rating = (rating_block, code_arrived) =>{
    ...
    var ref = firebase.database().ref('medical laboratory');
    console.log(ref);
...
const writeUserData = (userId ,code_send, email_send, message_send, name_send, phone_send, orignal, img_send) => {
    firebase.database().ref(`medical laboratory/${userId}`).set({
