...
var rootref = firebase.database().ref().child("Accountant");
...
var formref = firebase.database().ref('Accountant');
...
const close_rating = (rating_block, code_arrived) =>{
    ...
    var ref = firebase.database().ref('Accountant');
    console.log(ref);
...
const writeUserData = (userId ,code_send, email_send, message_send, name_send, phone_send, orignal) => {
    firebase.database().ref(`Accountant/${userId}`).set({
