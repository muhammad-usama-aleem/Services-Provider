...
var rootref = firebase.database().ref().child("nurse");
...
rootref.on("child_added", snap =>{
    ...
    let usersRef2 = firebase.database().ref('Services/nurse');
    ...
    usersRef.orderByValue().on("value", function(snapshot) {
        ...
        // current_id = snapshot.val().nurse.id;
        snapshot.forEach(function(data) {
            var data_id = data.val().id;
            // console.log(data.val())
            var all_data = data.val();
            // if(data.val().name !== "nurse"){
            // console.log("i ma here")

            // }
            
            if(current_id === data_id && data.val().name !== "nurse"){
                let div = document.createElement('div');
...
var formref = firebase.database().ref('nurse');
...
const close_rating = (rating_block, code_arrived) =>{
    ...
    var ref = firebase.database().ref('nurse');
    console.log(ref);
...
const writeUserData = (userId ,code_send, email_send, message_send, name_send, phone_send, orignal, img_send) => {
    firebase.database().ref(`nurse/${userId}`).set({
