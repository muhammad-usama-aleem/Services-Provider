...
var rootref = firebase.database().ref().child("creative writers");
...
rootref.on("child_added", snap =>{
    ...
    let usersRef2 = firebase.database().ref('Services/creative writers');
    ...
    usersRef.orderByValue().on("value", function(snapshot) {
        ...
        // current_id = snapshot.val().creative writers.id;
        snapshot.forEach(function(data) {
            var data_id = data.val().id;
            // console.log(data.val())
            var all_data = data.val();
            // if(data.val().name !== "creative writers"){
            // console.log("i ma here")

            // }
            
            if(current_id === data_id && data.val().name !== "creative writers"){
                let div = document.createElement('div');
...
var formref = firebase.database().ref('creative writers');
...
const close_rating = (rating_block, code_arrived) =>{
    ...
    var ref = firebase.database().ref('creative writers');
    console.log(ref);
...
const writeUserData = (userId ,code_send, email_send, message_send, name_send, phone_send, orignal, img_send) => {
    firebase.database().ref(`creative writers/${userId}`).set({
