...
var rootref = firebase.database().ref().child("Editor");
...
var formref = firebase.database().ref('Editor');
...
const close_rating = (rating_block, code_arrived) =>{
    ...
    var ref = firebase.database().ref('Editor');
    console.log(ref);
...
const writeUserData = (userId ,code_send, email_send, message_send, name_send, phone_send, orignal) => {
    firebase.database().ref(`Editor/${userId}`).set({
