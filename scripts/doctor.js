...
var rootref = firebase.database().ref().child("doctor");
...
rootref.on("child_added", snap =>{
    ...
    let usersRef2 = firebase.database().ref('Services/doctor');
    ...
    usersRef.orderByValue().on("value", function(snapshot) {
        ...
        // current_id = snapshot.val().doctor.id;
        snapshot.forEach(function(data) {
            var data_id = data.val().id;
            // console.log(data.val())
            var all_data = data.val();
            // if(data.val().name !== "doctor"){
            // console.log("i ma here")

            // }
            
            if(current_id === data_id && data.val().name !== "doctor"){
                let div = document.createElement('div');
...
var formref = firebase.database().ref('doctor');
...
const close_rating = (rating_block, code_arrived) =>{
    ...
    var ref = firebase.database().ref('doctor');
    console.log(ref);
