...
var rootref = firebase.database().ref().child("content writers");
...
rootref.on("child_added", snap =>{
    ...
    let usersRef2 = firebase.database().ref('Services/content writers');
    ...
    usersRef.orderByValue().on("value", function(snapshot) {
        ...
        // current_id = snapshot.val().content writers.id;
        snapshot.forEach(function(data) {
            var data_id = data.val().id;
            var all_data = data.val();
            
            if(current_id === data_id && data.val().name !== "content writers"){
                let div = document.createElement('div');
...
var formref = firebase.database().ref('content writers');
...
const close_rating = (rating_block, code_arrived) =>{
    ...
    var ref = firebase.database().ref('content writers');
    console.log(ref);
...
const writeUserData = (userId ,code_send, email_send, message_send, name_send, phone_send, orignal, img_send) => {
    firebase.database().ref(`content writers/${userId}`).set({
