...
var rootref = firebase.database().ref().child("copywriter");
...
var formref = firebase.database().ref('copywriter');
...
const close_rating = (rating_block, code_arrived) =>{
    ...
    var ref = firebase.database().ref('copywriter');
    console.log(ref);
...
const writeUserData = (userId ,code_send, email_send, message_send, name_send, phone_send, orignal) => {
    firebase.database().ref(`copywriter/${userId}`).set({
