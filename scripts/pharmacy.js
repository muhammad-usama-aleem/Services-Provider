...
var rootref = firebase.database().ref().child("pharmacy");
...
rootref.on("child_added", snap =>{
    ...
    let usersRef2 = firebase.database().ref('Services/pharmacy');
    ...
    usersRef.orderByValue().on("value", function(snapshot) {
        ...
        // current_id = snapshot.val().pharmacy.id;
        snapshot.forEach(function(data) {
            var data_id = data.val().id;
            // console.log(data.val())
            var all_data = data.val();
            // if(data.val().name !== "pharmacy"){
            // console.log("i ma here")

            // }
            
            if(current_id === data_id && data.val().name !== "pharmacy"){
                let div = document.createElement('div');
...
var formref = firebase.database().ref('pharmacy');
...
const writeUserData = (userId ,code_send, email_send, message_send, name_send, phone_send, orignal, img_send) => {
    firebase.database().ref(`pharmacy/${userId}`).set({
