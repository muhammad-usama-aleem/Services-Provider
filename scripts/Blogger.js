...
var rootref = firebase.database().ref().child("Blogger");
...
var formref = firebase.database().ref('Blogger');
...
const close_rating = (rating_block, code_arrived) =>{
    ...
    var ref = firebase.database().ref('Blogger');
    console.log(ref);
...
const writeUserData = (userId ,code_send, email_send, message_send, name_send, phone_send, orignal) => {
    firebase.database().ref(`Blogger/${userId}`).set({
